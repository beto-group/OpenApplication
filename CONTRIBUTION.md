# Contribution Guidelines — OpenApplication
...
```text
OpenApplication/
├── OPEN APPLICATION.md    # Obsidian entry point
...
└── src/
    ...
    ├── App.jsx            # Main coordinator loading dependency modules
    ├── components/
    │   ├── AppCard.jsx    # Individual app card tile with hover state and loading overlay
    │   └── AppLauncher.jsx # Core launcher UI with search, admin toggle, and grid
    └── styles/
...
3. **No-Polling Code Watcher**: The index bootstrapper registers an event listener with `app.vault.on("modify")` targeting files under `OpenApplication/src/`. This triggers an instant reload of the component's React view when source code modifications are saved, bypassing background CPU polling entirely.
